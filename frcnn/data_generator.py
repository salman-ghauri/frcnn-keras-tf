from frcnn import data_augmentation
import numpy as np
import cv2, random
import threading
import itertools
...
def iou(a, b):
	...
	uni = union(a, b, intersect)
	return float(intersect) / float(uni + 1e-6)
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
	...
	for idx in range(num_anchors_for_bbox.shape[0]):
		if num_anchors_for_bbox[idx] == 0:
			...
			y_rpn_overlap[best_anchor_for_bbox[idx, 0], best_anchor_for_bbox[idx, 1], best_anchor_for_bbox[idx, 2] + n_anchratios *
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				if mode == 'train':
					img_data_aug, x_img = data_augmentation.augment(img_data, C, augment=True)
				else:
					img_data_aug, x_img = data_augmentation.augment(img_data, C, augment=False)
